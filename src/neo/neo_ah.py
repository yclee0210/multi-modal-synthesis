...
class NeoAH(object):
    ...
    def set_specs(self, examples):
        ...
        self.likely = self.dsl.get_likely(self.abs_specs)
    ...
    def decide(self, program):
        ...
        return self.rank(fills, used)

    def rank(self, fills, used):
        lambdas = list()
        likely = list()
        others = list()
        for hole, component, score in fills:
            ...
            elif component in self.dsl.functions and component.name in self.likely and component not in used:
                likely.append((hole, component, score))
            else:
                others.append((hole, component, score))
        lambdas.sort(key=lambda x: -x[2])
        likely.sort(key=lambda x: -x[2])
        others.sort(key=lambda x: -x[2])
        return likely + others + lambdas
